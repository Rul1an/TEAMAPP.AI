...
- Advanced Analytics: heatmaps/predictions (na events-standaardisatie).
  ...
  - Export: Heatmap PNG‑export (RepaintBoundary) en CSV‑export van sparse counts (`row,col,count`). CSV respecteert k‑anonimiteit en toegepaste DP‑noise; predictions worden niet geëxporteerd. Metadata toegevoegd in CSV‑header (rows, cols, palette, k, dpEnabled/epsilon, category) en bestandsnamen bevatten timestamp en settings.
