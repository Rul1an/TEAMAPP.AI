...
-- Guards against missing auth schema
-- =============================================

-- Function to safely create policies only if tables exist AND auth schema exists
DO $$
BEGIN
  -- Check if auth schema exists before creating any auth-dependent policies
  IF EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = 'auth') THEN
    RAISE NOTICE '==============================================';
    RAISE NOTICE '🔐 AUTH SCHEMA FOUND - Creating authenticated policies';
    RAISE NOTICE '==============================================';
    ...
    RAISE NOTICE '==============================================';
    RAISE NOTICE '✅ RLS POLICIES ESTABLISHED (AUTH-ENABLED)';
  ...
  ELSE
    -- Auth schema doesn't exist - skip auth-dependent policies but create basic ones
    RAISE NOTICE '==============================================';
    RAISE NOTICE '⚠️ AUTH SCHEMA NOT FOUND - Local development mode';
    RAISE NOTICE '⚠️ Skipping authenticated user policies';
    RAISE NOTICE '⚠️ Creating basic anonymous policies only';
    RAISE NOTICE '==============================================';

    -- Create only anonymous access policies for development
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'videos') THEN
      DROP POLICY IF EXISTS "Videos: Anonymous users can view public videos" ON public.videos;
      CREATE POLICY "Videos: Anonymous users can view public videos" ON public.videos
        FOR SELECT TO anon
        USING (is_public = true);
      RAISE NOTICE '✅ Created basic video anonymous policy';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'training_exercises') THEN
      ...
      CREATE POLICY "Training Exercises: Anonymous users can view public exercises" ON public.training_exercises
        FOR SELECT TO anon
        USING (is_public = true);
      RAISE NOTICE '✅ Created basic training exercises anonymous policy';
    END IF;

    RAISE NOTICE '==============================================';
    RAISE NOTICE '✅ BASIC RLS POLICIES ESTABLISHED (NO AUTH)';
    RAISE NOTICE '✅ Anonymous access policies created';
    RAISE NOTICE '✅ Auth policies will be applied when deployed to managed Supabase';
    RAISE NOTICE '✅ Safe for local development environment';
    RAISE NOTICE '==============================================';
  END IF;
END
$$;
