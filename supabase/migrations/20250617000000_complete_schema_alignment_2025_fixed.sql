-- =============================================
-- COMPLETE SCHEMA ALIGNMENT MIGRATION 2025 (FIXED)
-- Adds all missing columns, views, and conditional storage policies
-- Follows Supabase 2025 best practices with local/managed compatibility
-- =============================================
...
-- Conditionally create storage policies only if both storage schema AND storage.folder function exist
DO $$
DECLARE
  has_storage_folder_function boolean := false;
BEGIN
  RAISE NOTICE '==============================================';
  ...
  IF EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = 'storage') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'storage' AND table_name = 'objects') THEN
      RAISE NOTICE '✅ Storage.objects table found';

      -- CRITICAL FIX: Check if storage.folder function exists before using it
      SELECT EXISTS (
        SELECT 1 FROM information_schema.routines
        WHERE routine_schema = 'storage'
        AND routine_name = 'folder'
        AND routine_type = 'FUNCTION'
      ) INTO has_storage_folder_function;

      IF has_storage_folder_function THEN
        RAISE NOTICE '✅ Storage.folder function found - creating RLS policies with folder support';

        -- Video storage policies (only for managed environments with storage.folder function)
        ...
        RAISE NOTICE '✅ Storage RLS policies with folder support created successfully';

      ELSE
        RAISE NOTICE 'ℹ️ Storage.folder function not found - creating basic storage policies';

        -- Fallback: Create basic storage policies without folder function
        -- These policies work but are less restrictive
        DROP POLICY IF EXISTS "Videos: Basic authenticated access" ON storage.objects;
        CREATE POLICY "Videos: Basic authenticated access" ON storage.objects
          FOR ALL TO authenticated
          USING (bucket_id = 'videos');

        DROP POLICY IF EXISTS "Videos: Basic anonymous read access" ON storage.objects;
        CREATE POLICY "Videos: Basic anonymous read access" ON storage.objects
          FOR SELECT TO anon
          USING (bucket_id = 'videos');

        RAISE NOTICE '✅ Basic storage RLS policies created successfully';
      END IF;
...
BEGIN
  RAISE NOTICE '==============================================';
  RAISE NOTICE '✅ COMPLETE SCHEMA ALIGNMENT SUCCESSFUL (FIXED)';
  RAISE NOTICE '==============================================';
  ...
  RAISE NOTICE '   • Applied conditional storage policies (with function availability detection)';
  ...
  RAISE NOTICE '   • RLS policies adapt to function availability';
