...
class PDFService {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
            ...
            _buildPlayersSection(session, allPlayers, primaryColor, backgroundColor),
            pw.SizedBox(height: 16),
  ...
  static pw.Widget _buildTrainingHeader(TrainingSession session, PdfColor primaryColor) => pw.Container(
  ...
  static pw.Widget _buildTrainingInfoSection(TrainingSession session, PdfColor primaryColor, PdfColor backgroundColor) => pw.Container(
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    _buildInfoRow('Datum:', DateFormat('dd-MM-yyyy').format(session.date)),
                    pw.SizedBox(height: 6),
                    _buildInfoRow('Training Nr:', session.trainingNumber.toString()),
                    pw.SizedBox(height: 6),
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.SizedBox(height: 6),
                    _buildInfoRow('Fasen:', '${session.phases.length} training fasen'),
                    pw.SizedBox(height: 6),
  ...
  static pw.Widget _buildPlayersSection(TrainingSession session, List<Player> allPlayers, PdfColor primaryColor, PdfColor backgroundColor) {
    final presentPlayers = allPlayers.where((p) => session.playerAttendance.values.any((a) => a.playerId == p.id.toString() && a.status == AttendanceStatus.present)).toList();
    final absentPlayers = allPlayers.where((p) => session.playerAttendance.values.any((a) => a.playerId == p.id.toString() && a.status == AttendanceStatus.absent)).toList();
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    ...
                    ...presentPlayers.map((player) => pw.Padding(
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.SizedBox(height: 8),
                    ...absentPlayers.map((player) => pw.Padding(
                      padding: const pw.EdgeInsets.only(bottom: 3),
                      child: pw.Text(
  ...
  static pw.Widget _buildObjectivesSection(TrainingSession session, PdfColor primaryColor, PdfColor backgroundColor) => pw.Container(
      padding: const pw.EdgeInsets.all(16),
      decoration: pw.BoxDecoration(
        color: backgroundColor,
        borderRadius: pw.BorderRadius.circular(8),
      ...
      child: pw.Column(
        ...
        children: [
          ...
          pw.SizedBox(height: 12),
          _buildObjectiveItem('Training Doelstelling:', session.sessionObjective ?? 'Technisch-tactische ontwikkeling'),
          pw.SizedBox(height: 8),
          _buildObjectiveItem('Team Functie:', session.teamFunction ?? 'Positiespel en samenwerking'),
          pw.SizedBox(height: 8),
          _buildObjectiveItem('Coaching Accent:', session.coachingAccent ?? 'Individuele begeleiding'),
        ],
      ),
    );

  static pw.Widget _buildTrainingPhasesSection(TrainingSession session, PdfColor primaryColor, PdfColor backgroundColor) => pw.Container(
      padding: const pw.EdgeInsets.all(16),
      decoration: pw.BoxDecoration(
        color: backgroundColor,
        borderRadius: pw.BorderRadius.circular(8),
  ...
  static pw.Widget _buildTrainingFooter(TrainingSession session, PdfColor greyColor) => pw.Container(
      padding: const pw.EdgeInsets.all(12),
      decoration: pw.BoxDecoration(
  ...
  static pw.Widget _buildObjectiveItem(String label, String content) => pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(
          label,
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
            ...
            pw.SizedBox(height: 20),
            _buildAssessmentOverviewCard(assessment, primaryColor, backgroundColor),
            pw.SizedBox(height: 20),
  ...
  static pw.Widget _buildHeader(Player player, PlayerAssessment assessment, PdfColor primaryColor) => pw.Container(
      padding: const pw.EdgeInsets.all(20),
      decoration: pw.BoxDecoration(
        color: primaryColor,
        borderRadius: pw.BorderRadius.circular(8),
  ...
  static pw.Widget _buildPlayerInfoCard(Player player, PdfColor primaryColor, PdfColor backgroundColor) => pw.Container(
      padding: const pw.EdgeInsets.all(16),
      decoration: pw.BoxDecoration(
        color: backgroundColor,
        borderRadius: pw.BorderRadius.circular(8),
          ...
          pw.Row(
            children: [
              pw.Expanded(
                child: pw.Column(
                  children: [
                    ...
                    pw.SizedBox(height: 8),
                    _buildInfoItem('Positie', _getPositionName(player.position)),
                    pw.SizedBox(height: 8),
              ...
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _buildInfoItem('Geboortedatum', DateFormat('dd-MM-yyyy').format(player.birthDate)),
                    pw.SizedBox(height: 8),
                    _buildInfoItem('Lengte', '${player.height} cm'),
                    pw.SizedBox(height: 8),
                    _buildInfoItem('Voorkeurvoet', player.preferredFoot == PreferredFoot.left ? 'Links' : 'Rechts'),
                  ],
                ),
              ),
            ],
  ...
  static pw.Widget _buildAssessmentOverviewCard(PlayerAssessment assessment, PdfColor primaryColor, PdfColor backgroundColor) => pw.Container(
      padding: const pw.EdgeInsets.all(16),
      decoration: pw.BoxDecoration(
        color: backgroundColor,
        borderRadius: pw.BorderRadius.circular(8),
          ...
          pw.Row(
            children: [
              ...
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _buildCategoryRating('Technisch', assessment.technicalAverage, const PdfColor.fromInt(0xFF2196F3)),
                    pw.SizedBox(height: 8),
                    _buildCategoryRating('Tactisch', assessment.tacticalAverage, const PdfColor.fromInt(0xFF4CAF50)),
                    pw.SizedBox(height: 8),
                    _buildCategoryRating('Fysiek', assessment.physicalAverage, const PdfColor.fromInt(0xFFFF9800)),
                    pw.SizedBox(height: 8),
                    _buildCategoryRating('Mentaal', assessment.mentalAverage, const PdfColor.fromInt(0xFF9C27B0)),
                  ],
                ),
              ),
            ],
  ...
  static pw.Widget _buildOverallRatingBox(double rating) {
    final color = rating >= 4.0 ? const PdfColor.fromInt(0xFF4CAF50) :
                  rating >= 3.0 ? const PdfColor.fromInt(0xFFFF9800) :
  ...
  static pw.Widget _buildCategoryRating(String category, double rating, PdfColor color) => pw.Row(
      children: [
        pw.Container(
          width: 12,
          height: 12,
  ...
  static pw.Widget _buildSkillsBreakdownCard(PlayerAssessment assessment, PdfColor primaryColor, PdfColor backgroundColor) => pw.Container(
      padding: const pw.EdgeInsets.all(16),
      decoration: pw.BoxDecoration(
        color: backgroundColor,
        borderRadius: pw.BorderRadius.circular(8),
          ...
          pw.Row(
            ...
            children: [
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _buildSkillCategory('Technische Vaardigheden', assessment.technicalSkills, const PdfColor.fromInt(0xFF2196F3)),
                    pw.SizedBox(height: 16),
                    _buildSkillCategory('Tactische Vaardigheden', assessment.tacticalSkills, const PdfColor.fromInt(0xFF4CAF50)),
                  ],
                ),
              ),
              pw.SizedBox(width: 16),
              pw.Expanded(
                child: pw.Column(
                  children: [
                    _buildSkillCategory('Fysieke Eigenschappen', assessment.physicalAttributes, const PdfColor.fromInt(0xFFFF9800)),
                    pw.SizedBox(height: 16),
                    _buildSkillCategory('Mentale Eigenschappen', assessment.mentalAttributes, const PdfColor.fromInt(0xFF9C27B0)),
                  ],
                ),
              ),
            ],
  ...
  static pw.Widget _buildSkillCategory(String categoryName, Map<String, int> skills, PdfColor color) => pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Container(
          padding: const pw.EdgeInsets.symmetric(horizontal: 8, vertical: 4),
        ...
        pw.SizedBox(height: 8),
        ...skills.entries.map((entry) => pw.Padding(
  ...
  static pw.Widget _buildStarRating(int rating) => pw.Row(
      ...
      children: List.generate(5, (index) => pw.Container(
          ...
          decoration: pw.BoxDecoration(
            color: index < rating ? const PdfColor.fromInt(0xFFFFD700) : const PdfColor.fromInt(0xFFE0E0E0),
            shape: pw.BoxShape.circle,
          ),
        ),),
    );

  static pw.Widget _buildTextFeedbackCard(PlayerAssessment assessment, PdfColor primaryColor, PdfColor backgroundColor) {
      ...
      child: pw.Column(
        ...
        children: [
          ...
          if (assessment.strengths?.isNotEmpty ?? false)
            _buildTextSection('Sterke Punten', assessment.strengths!, const PdfColor.fromInt(0xFF4CAF50)),
          if (assessment.areasForImprovement?.isNotEmpty ?? false) ...[
            pw.SizedBox(height: 12),
            _buildTextSection('Verbeterpunten', assessment.areasForImprovement!, const PdfColor.fromInt(0xFFFF9800)),
          ],
          if (assessment.developmentGoals?.isNotEmpty ?? false) ...[
            pw.SizedBox(height: 12),
            _buildTextSection('Ontwikkelingsdoelen', assessment.developmentGoals!, const PdfColor.fromInt(0xFF2196F3)),
          ],
          if (assessment.coachNotes?.isNotEmpty ?? false) ...[
            pw.SizedBox(height: 12),
            _buildTextSection('Coach Notities', assessment.coachNotes!, const PdfColor.fromInt(0xFF9C27B0)),
          ],
        ],
      ),
    );
  ...
  static pw.Widget _buildTextSection(String title, String content, PdfColor color) => pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Container(
          padding: const pw.EdgeInsets.symmetric(horizontal: 8, vertical: 4),
  ...
  static pw.Widget _buildFooter(PlayerAssessment assessment, PdfColor greyColor) => pw.Container(
      padding: const pw.EdgeInsets.all(12),
      decoration: pw.BoxDecoration(
        border: pw.Border(top: pw.BorderSide(color: greyColor)),
      ),
