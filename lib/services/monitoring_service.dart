...
class MonitoringService {
  static const String _sentryDsn = String.fromEnvironment(
    'SENTRY_DSN',
    defaultValue: '', // Will be configured in production
  );
...
mixin MonitoringMixin {
  Future<void> trackEvent(String name, {Map<String, dynamic>? parameters}) {
    return MonitoringService.trackEvent(name: name, parameters: parameters);
  }

  Future<void> trackError(dynamic error, {StackTrace? stackTrace, String? context}) {
    return MonitoringService.reportError(
      error: error,
      stackTrace: stackTrace,
      context: context,
    );
  ...
  Future<T> monitorOperation<T>(String operation, Future<T> Function() function) {
