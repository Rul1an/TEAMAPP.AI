...
class _SessionBuilderScreenState extends ConsumerState<SessionBuilderScreen> {
  ...
  final TextEditingController _coachingAccentController = TextEditingController();
  ...
  void _createNewSession() {
    session = TrainingSession.create(
      teamId: 'jo17-1',
      date: selectedDate,
      trainingNumber: DateTime.now().millisecondsSinceEpoch ~/ (1000 * 60 * 60 * 24), // Simple unique number
      type: selectedType,
    );
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Team info
                  ListTile(
                    leading: Icon(Icons.groups, color: Theme.of(context).primaryColor),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ListTile(
                            ...
                            subtitle: const Text('18:00 - 20:10 (130 minuten)'),
                            trailing: TextButton(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintText: 'Bijv: Communicatie, Positiespel, Druk zetten',
                      border: OutlineInputBorder(),
                    ),
                  ),
                ],
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    runSpacing: 8,
                    children: [
                      _buildPhaseTemplate('Korte Warming-up', PhaseType.warmup, 10),
                      _buildPhaseTemplate('Techniek Blok', PhaseType.technical, 20),
                      _buildPhaseTemplate('Tactiek Vorm', PhaseType.tactical, 25),
                      _buildPhaseTemplate('Partijtje', PhaseType.game, 15),
                      _buildPhaseTemplate('Extra Uitloop', PhaseType.cooldown, 5),
                    ],
                  ),
                ],
              ),
                ...
                children: [
                  ...
                  if (_getTotalDuration() != 130)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _getTotalDuration() > 130 ? Colors.orange : Colors.blue,
  ...
  void _recalculatePhaseTimes() {
    ...
    for (int i = 0; i < sessionPhases.length; i++) {
      ...
      sessionPhases[i].endTime = currentTime.add(Duration(minutes: sessionPhases[i].durationMinutes));
  ...
  void _deletePhase(int index) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Weet je zeker dat je "${sessionPhases[index].name}" wilt verwijderen?'),
        actions: [
          TextButton(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSummaryRow('Datum', '${selectedDate.day}/${selectedDate.month}/${selectedDate.year}'),
                  _buildSummaryRow('Type', _getTrainingTypeDisplayName(selectedType)),
                  _buildSummaryRow('Duur', '${_getTotalDuration()} minuten'),
                  _buildSummaryRow('Fasen', '${sessionPhases.length} fasen'),
                  _buildSummaryRow('Doelstelling', _objectiveController.text.isEmpty ? 'Niet ingevuld' : _objectiveController.text),
                ],
              ),
            ),
          ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: 'Voeg extra notities toe voor deze training...',
                      border: OutlineInputBorder(),
                    ),
                    maxLines: 4,
                  ),
  ...
  void _selectTiming() {
    // TODO: Implement custom timing selection
  }
  ...
  void _editPhase(int index) {
    ...
    final durationController = TextEditingController(text: phase.durationMinutes.toString());
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<PhaseType>(
                  ...
                  items: PhaseType.values.map((type) => DropdownMenuItem(
                    value: type,
                    child: Row(
                      children: [
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                final duration = int.tryParse(durationController.text) ?? phase.durationMinutes;
                final updatedPhase = sessionPhases[index].copyWith(
                  name: nameController.text.trim().isEmpty ? phase.name : nameController.text.trim(),
                  type: selectedType,
                  description: descriptionController.text.trim(),
                );

                // Manually update the duration by setting new end time
                updatedPhase.endTime = updatedPhase.startTime.add(Duration(minutes: duration));
                ...
                DropdownButtonFormField<PhaseType>(
                  ...
                  onChanged: (value) {
                    if (value != null) {
                      setDialogState(() {
                        selectedType = value;
                        descriptionController.text = _getDefaultPhaseDescription(value);
                      });
                    }
                  },
                ),
  ...
  Future<void> _exportToPDF() async {
    ...
    try {
      ...
      // Generate PDF
      final pdfData = await PDFService.generateTrainingSessionReport(session!, allPlayers);
      ...
      if (kIsWeb) {
        ...
        final filePath = '${downloadsDir?.path ?? (await getApplicationDocumentsDirectory()).path}/$fileName';
  ...
  void _addExerciseToPhaseQuick(SessionPhase phase) async {
    ...
    if (session!.id == 0) {
      ...
      if (mounted && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Sla de sessie eerst op voordat je oefeningen toevoegt'),
            backgroundColor: Colors.orange,
          ),
        );
      }
